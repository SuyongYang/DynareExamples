...
//Consumption Euler equation
 (1/exp(lc)) = beta*(1/exp(lc(+1)))*(1+alpha*(exp(lk)^(alpha-1))
  *exp(z(+1))*exp(lh(+1))^(1-alpha)-delta);
...
//Capital rental rate
 %exp(Rk) = alpha*(exp(ly)/exp(lk(-1))^(1-alpha)); %(modified by me)
  exp(Rk) = alpha*exp(ly)/exp(lk(-1)); %(original Miao)
