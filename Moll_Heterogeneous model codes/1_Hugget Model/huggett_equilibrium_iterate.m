...
%INITIAL GUESS
r = r0;
v0(:,1) = (z(1) + r.*a).^(1-s)/(1-s)/rho; % Achdou et al(2017) appendix
...
for n=1:maxit
    ...
    c = cf.*If + cb.*Ib + c0.*I0; % art of programming :)
    u = c.^(1-s)/(1-s);
    
    %CONSTRUCT MATRIX Moll lec. ch3 p22
    X = -min(ssb,0)/da;
...
%[gg,val]= eigs(AT,1,'lr'); %lr : largest real value
%g_sum = gg'*ones(2*I,1)*da;
%gg = gg./g_sum;
