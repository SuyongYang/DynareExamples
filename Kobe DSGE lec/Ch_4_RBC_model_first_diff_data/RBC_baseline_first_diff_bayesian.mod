...
/*
 * Copyright (C) 2014-2018 Johannes Pfeifer
 *
...
@# if use_real_data==1
    stoch_simul(order=1,periods=0)  g_obs c_obs;
@# else
...
@# if use_real_data==1
    estimation(datafile=first_diff_real_data,mh_jscale=1.5,mode_check,
...
@# else
    estimation(datafile=first_diff_data_simulated,mh_jscale=1.5,mode_check,
        mh_replic=4000,mh_nblocks=1,bayesian_irf,smoother,conditional_variance_decomposition=[1 2 4]) log_y log_k log_c log_l log_w r z ghat g_obs c_obs;
@# endif
